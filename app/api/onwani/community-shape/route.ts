...
export async function GET(req: Request) {
  ...
  const community = searchParams.get("community") || searchParams.get("community_name_en");

  if (!municipality || !district || !community) {
    return NextResponse.json(
      { error: "Missing required parameters: municipality, district and community" },
      { status: 400 }
    );
  }

  try {
    const upstream = new URL("https://onwani.abudhabi.ae/tamm/api/getcommunityshape");
    upstream.searchParams.set("municipality", municipality);
  upstream.searchParams.set("DISTRICT_NAME_EN", district);
  upstream.searchParams.set("community_name_en", community);
