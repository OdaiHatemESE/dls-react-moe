...
// Prefer IPv4 first to avoid IPv6 resolution issues in some environments
try {
...
export async function GET(req: Request) {
  ...
  if (!municipality) {
    return NextResponse.json({ error: "Missing required parameter: municipality" }, { status: 400 });
  }

  try {
      const upstream = new URL("https://onwani.abudhabi.ae/tamm/api/getdistricts");
    ...
    try {
      ...
      // ignore CA load errors; fall back to default trust store
    }
    ...
    const result: UpstreamResult = await new Promise((resolve) => {
      ...
      req.on("error", (err) => {
        resolve({ ok: false, status: 502, statusText: "Bad Gateway", body: { error: err.message } });
      });
      req.end();
    });
    ...
    return NextResponse.json(
      {
        error: message,
        code,
        note: "Upstream fetch failed",
        hint: "If this persists, check IPv6/TLS from the host or run with IPv4-first DNS.",
      },
      { status: 500 }
    );
  }
