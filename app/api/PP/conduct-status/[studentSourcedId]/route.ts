...
export async function PATCH(
  ...
  const endpoint = '/api/PP/conduct-status/[studentSourcedId]';
  ...
  try {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES + 1; attempt++) {
      try {
        ...
        console.error(`[PP Conduct Status] API call attempt ${attempt} failed:`, err.message);
    ...
    if (!ppRes) {
      return NextResponse.json(
        { error: 'Failed to update conduct status after retries' },
        { status: 503 }
      );
    }
